...
public class WaveView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            mHeight = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 200, mContext.getResources().getDisplayMetrics());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(mMode == MODE_TRIANGLE) {
            ...
            path.moveTo(startX, startY);
            for (int i = 0; i < mWaveCount; i++) {
                path.lineTo(startX - mWaveWidth, padding+(i*mWaveHeight)+(mWaveHeight/2));
                path.lineTo(startX, startY + mWaveHeight * (i + 1));
            }
